...
exports.sendChatMessageNotification = exports.sendSilentTrackRequest = exports.getCoOccurrences = exports.logTransition = exports.getCollabNextTrack = exports.getTrending = exports.logPlayback = void 0;
...
const chatDb = (0, firestore_2.getFirestore)('chat');
...
function getClientIp(req) {
    ...
    return req.ip || ((_a = req.socket) === null || _a === void 0 ? void 0 : _a.remoteAddress) || 'unknown_ip';
}
// ─────────────────────────────────────────────────────────────────────────────
// FUNCTION 1: Log Playback (IP-Based User Identity)
// ─────────────────────────────────────────────────────────────────────────────
exports.logPlayback = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'POST') {
    ...
    try {
        ...
        if (!(trackObject === null || trackObject === void 0 ? void 0 : trackObject.id) || !(trackObject === null || trackObject === void 0 ? void 0 : trackObject.title)) {
...
// ─────────────────────────────────────────────────────────────────────────────
exports.getTrending = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'GET') {
        res.status(405).json({ success: false, message: 'Method Not Allowed' });
        return;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
exports.getCollabNextTrack = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'GET') {
        res.status(405).json({ success: false, message: 'Method Not Allowed' });
        return;
    }
    try {
        ...
        const nextTrackVotes = {};
        if (!matchesSnapshot.empty) {
            // Map of clientIp -> timestamps
            const usersListenTimes = new Map();
        ...
        const searchQueries = recentSearches.docs.map((doc) => { var _a; return ((_a = doc.data().query) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || ''; });
        ...
        let topCandidates = Object.values(nextTrackVotes)
            ...
            return Object.assign(Object.assign({}, item.metadata), { votes: item.count, globalPlays: item.globalPlays, score: blendedScore });
        })
        ...
        if (topCandidates.length < 5) {
            ...
            const trendingFallback = Object.values(trendingFrequencies)
                ...
                .map((item) => (Object.assign(Object.assign({}, item.metadata), { globalPlays: item.count, score: item.count * 0.3 })));
...
// ─────────────────────────────────────────────────────────────────────────────
exports.logTransition = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ success: false, message: 'Method Not Allowed' });
        return;
    }
    try {
        ...
        if (!sourceTrackId || !(destTrack === null || destTrack === void 0 ? void 0 : destTrack.id) || !(destTrack === null || destTrack === void 0 ? void 0 : destTrack.title)) {
...
// ─────────────────────────────────────────────────────────────────────────────
exports.getCoOccurrences = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'GET') {
        res.status(405).json({ success: false, message: 'Method Not Allowed' });
        return;
    }
    try {
        ...
        const results = Object.values(transitionCounts)
            .sort((a, b) => b.count - a.count)
            .slice(0, 10)
            .map((item) => (Object.assign(Object.assign({}, item.metadata), { transitionCount: item.count })));
...
// ─────────────────────────────────────────────────────────────────────────────
exports.sendSilentTrackRequest = (0, firestore_1.onDocumentUpdated)('live_users/{uid}', async (event) => {
    var _a, _b, _c, _d;
...
// ─────────────────────────────────────────────────────────────────────────────
exports.sendChatMessageNotification = (0, firestore_1.onDocumentCreated)({
